...
var defaultAppName = argv.name ? argv.name : 'sy';
...
gulp.task('uglify', ['uglify:react', 'uglify:react-bootstrap', 'uglify:app']);
...
gulp.task('build:essentials', ['react', 'react-bootstrap']);
...
gulp.task("buildBuild",function(callback){
    runSequence('build:css', 'build:essentials', 'build:app', 'base64-css', callback);
});

gulp.task('copyPublic',['buildBuild'],function(){
    gulp.src(["./public/**/*","!./public/demo/**/*","!./public/css/app/**/*","!./public/css/demo/**/*","!./public/js/app/**/*","!./public/js/demo/**/*","!./public/locales/demo/**/*","!./public/locales/demo/**/*"])
        .pipe(copyExt())
        .pipe(gulp.dest(dist+'/static-admin'));
});

gulp.task("buildIndex",function(){
    //生成入口文件
    gulp.src("src/jsx/sy/index_build.html")
        .pipe(copyExt())
...
// 可视化
gulp.task("buildPreview",function(){

});

//build App
gulp.task('build', ['cleanStatic'], function(){
    gulp.start(['buildIndex','copyPublic']);
});
