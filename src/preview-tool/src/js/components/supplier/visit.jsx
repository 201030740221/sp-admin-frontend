        ...
        formData: [
          {
            ...
            title: '拜访时间',
            key: 'visit_time',
            defaultValue: defaultValue.visit_time || ''
          }, {
            type: 'textarea',
            title: '拜访目的与结果',
            key: 'visit_result',
            defaultValue: defaultValue.visit_result || '',
            validator: {
              required: true,
              message: {
                required: '必填'
...
var Visit = React.createClass({
  ...
  remove: function (id) {
    webapi.supplier.removeVisitRecord({id: id}).then(function (res) {
      if (res.code === 0) {
  ...
  render: function () {
    ...
    var data = {
      columns: columns,
      url: webapi.supplier.visitListUrl,
      params: {
