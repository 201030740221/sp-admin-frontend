    ...
    UploadImage.prototype = {
        ...
        initUploader: function () {
            ...
            // WebUploader实例
                ...
                // tofishes 增加一个上传成功的回调配置
                uploodSuccess = editor.getOpt('uploadSuccess') || function () {};
    ...
    OnlineImage.prototype = {
        ...
        initEvents: function () {
            ...
            domUtils.on(this.container, 'click', function (e) {
                ...
                if (!domUtils.hasClass(target, 'remove')) {
                    return;
                }
                e.stopPropagation();
                ...
                var media_id = target.getAttribute('media_id')
                ,   params = editor.getOpt('onlineImagesParams')
                ,   url = '/api/attachment/delete'
                ,   webapi = window.top.webapi;

                params.media_ids = [media_id];

                webapi.attachment.removeAttachment(params).then(function (res) {
                    if (!res || res.code!== 0) {
                        alert('删除失败：' + res.msg);
                        return;
                    };
        ...
        getImageData: function () {
            ...
            if(!_this.listEnd && !this.isLoadingData) {
                ...
                var url = editor.getActionUrl(editor.getOpt('imageManagerActionName')),
                    extraParams = editor.onlineImagesParams || editor.getOpt('onlineImagesParams'),
                    isJsonp = utils.isCrossDomainUrl(url);
                // tofishes 配置在线管理，添加额外参数
                var _data =  utils.extend({
                            start: this.listIndex,
                            size: this.listSize
                        }, editor.queryCommandValue('serverparam'));
                _data = utils.extend(_data, extraParams);
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    domUtils.addClass(remove, 'remove');
                    remove.setAttribute('media_id', list[i].media_id);
